...
%w(
pern xanth elenium mists_of_avalon buffy_the_vampire_slayer
).each{ |univ| Universe.create(:name => univ, :genre_id => Genre.find_by_name("Fantasy")) }

%w(
star_wars star_trek dr_who Ender's_game ringworld discworld wild_cards
...
%w(
shaddam corrino atreides alia ghanima leto muad'dib kwisatz_haderach bene_gesserit dune arrakis arrakeen
...
quizariat shadout_mapes Tupali varota
...
umma uroshnor batigh varite wali weirding yali suk
).each{ |word| Word.create(:word => word, :universe_id => dune.id)}

%w(
combine_honnete_ober_advancer_mercantiles bhotani_jib bi-la_kaifa lisan_al-gaib
).each{ |phrase| Phrase.create(:phrase => phrase, :universe_id => dune.id, :titleize => true)}